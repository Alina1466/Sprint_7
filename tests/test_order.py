...
import pytest
import user_data
from api_scooter.api_order import ApiOrder
from api_scooter.api_courier import ApiCourier


class TestOrder:
    @allure.title('Проверка создания заказа')
    def test_create_order_success(self):
        response = ApiOrder.create_order(user_data.payload_order)
    ...
    def test_crate_order_diff_colors_success(self, color):
        user_data.payload_order['color'] = color
        response = ApiOrder.create_order(user_data.payload_order)
    ...
    def test_accept_order_success(self, create_random_courier):
        ...
        order_track = ApiOrder.create_order(user_data.payload_order).json()['track']
